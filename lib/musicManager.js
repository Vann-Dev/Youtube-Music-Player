...
function timeUpdate() {
    position = Math.floor(document.getElementById('audio').currentTime) * 1000
}
...
export function play() {
    ...
    if (!track) return;
    currentTrack = track
    document.querySelector('#audio').src = track.audio;
...
export function addTrack(track) {
    queue.push({
        title: track.title,
        audio: `https://express-1.stevanvincent.repl.co/track/${track.id}`,
